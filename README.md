Sample code for custom COM marshaling with shared memory. This pattern can be used for <mark>**zero-copy exchange of large buffers between processes**</mark>. The implementation is _transparent_, meaning that clients doesn't notice that the COM `SAFEARRAY` buffers reside in shared memory.
